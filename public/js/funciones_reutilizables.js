...
// Function to generate a device ID based on hardware and software characteristics
function generateDeviceId() {
    ...
    // Use a hash function to create a more concise ID
    return simpleHash(deviceCharacteristics);
  }
  ...
  export function getTempUserIdentifier() {
    // Try to get the ID from localStorage
    let deviceId = localStorage.getItem('deviceId');
    
    if (!deviceId) {
      // If not in localStorage, try to get from a cookie
      deviceId = getCookie('deviceId');
      
      if (!deviceId) {
        // If not in cookie, generate a new ID
        deviceId = generateDeviceId();
        
        // Store the new ID in both localStorage and cookie
        localStorage.setItem('deviceId', deviceId);
        setCookie('deviceId', deviceId, 365); // Set cookie to expire in 1 year
      } else {
        // If found in cookie, also store in localStorage for future use
        localStorage.setItem('deviceId', deviceId);
      }
    } else {
      // If found in localStorage, also update the cookie
